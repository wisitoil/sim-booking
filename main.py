...
from fastapi.responses import HTMLResponse # เพิ่มตัวนี้เข้ามา
...
# ---- เพิ่มโค้ดส่วนนี้เพื่อให้ระบบดึงหน้าจอ index.html มาแสดงผลเมื่อเปิดเว็บ ----
@app.get("/", response_class=HTMLResponse, summary="หน้าจอจองคิวสำหรับลูกค้า")
def read_root():
    # ตรวจสอบตำแหน่งไฟล์ index.html
    template_path = os.path.join("templates", "index.html")
    if os.path.exists(template_path):
        with open(template_path, "r", encoding="utf-8") as file:
            return file.read()
    return "<h1>ไม่พบไฟล์หน้าจอระบบจองคิวในโฟลเดอร์ templates</h1>"
# ------------------------------------------------------------------------
